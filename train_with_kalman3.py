...
import numpy as np  # 🔧 必须在最开头导入numpy
...
class KalmanTrack:
    ...
    def __init__(self, x, y, n_init=1, max_age=5):  # 🔧 修复: n_init改为1
        ...
        # 🔧 关键修复: n_init=1时，新建的track立即confirmed
        self.n_init = n_init
...
class KalmanMOTATracker:
    ...
    def __init__(self, max_age=5, n_init=1, max_distance=5.0):  # 🔧 修复: n_init改为1
        self.max_age = max_age
        self.n_init = n_init
...
def train_epoch(model, dataloader, optimizer, criterion, device, scaler, epoch,
    ...
    sequence_tracker = SequenceMOTATracker(
        ...
        n_init=1,  # 🔧 关键修复: 改为1，因为训练数据是打乱的，无法连续跟踪
        max_distance=kalman_match_threshold  # 根据epoch调整
    )
    print(f"   📊 MOTA评估使用卡尔曼跟踪器 (max_age=5, n_init=1, threshold={kalman_match_threshold}m)")
