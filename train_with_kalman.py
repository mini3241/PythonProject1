...
class KalmanMOTATracker:
    ...
    def get_motp(self):
        if len(self.match_distances) == 0:
            return 0.0
        return np.mean(self.match_distances)
...
class SequenceMOTATracker:
    ...
    def print_global_metrics(self):
        ...
        print(f"   ✅ MOTA={mota * 100:.2f}%, MOTP={motp:.2f}m")
        print(f"{'=' * 60}\n")
    ...
    def _load_camera_data(self, frame_path):
        ...
        if os.path.exists(lidar_path):
            ...
            if json_files:
                ...
                # 🔧 计算LiDAR→OCU转换矩阵（修复：从label_data中读取两个extrinsic）
                transform_matrix = None
                try:
                    # 关键修复：两个extrinsic都在VelodyneLidar的JSON中！
                    if 'VelodyneLidar_to_LeopardCamera0_extrinsic' in label_data and \
                            'OCULiiRadar_to_LeopardCamera0_extrinsic' in label_data:
                        Lidar_to_Camera = np.array(label_data['VelodyneLidar_to_LeopardCamera0_extrinsic'])
                        OCU_to_Camera = np.array(label_data['OCULiiRadar_to_LeopardCamera0_extrinsic'])
                ...
                if not hasattr(self, '_gt_transform_debug_done'):
                    ...
                    print(f"\n🔴🔴🔴 GT坐标转换调试 🔴🔴🔴")
                    print(
                        f"   label_data有Lidar_extrinsic: {'VelodyneLidar_to_LeopardCamera0_extrinsic' in label_data}")
                    print(f"   label_data有OCU_extrinsic: {'OCULiiRadar_to_LeopardCamera0_extrinsic' in label_data}")
                ...
                if len(output.shape) >= 2:
                    ...
                    if len(coords[0]) > 0:
                        ...
                        for det_idx, (y, x) in enumerate(zip(coords[0], coords[1])):
                            ...
                            # 🔴🔴🔴 MOTA诊断断点 - 自动插入 🔴🔴🔴
                            if batch_num <= 10 and det_idx == 0 and len(targets) > 0:
...
def train_epoch(model, dataloader, optimizer, criterion, device, scaler, epoch,
    ...
    # 根据epoch调整匹配阈值
    if epoch <= 5:
        kalman_match_threshold = 10.0  # 训练初期用10米
    elif epoch <= 15:
        kalman_match_threshold = 5.0  # 中期用5米
    else:
        kalman_match_threshold = 2.5  # 后期用2.5米（baseline标准）
        ...
        try:
            ...
            if new_tracks_collected:
                ...
                if eval_interval and batch_idx % eval_interval == 0:
                    ...
                    print(
                        f"\n   📊 批次{batch_idx} | MOTA={batch_mota * 100:.2f}%, MOTP={batch_motp:.2f}m, IDSW={total_idsw}")
    ...
    if eval_mota:
        ...
        print(f"   ✅ 最终 MOTA: {mota * 100:.2f}%, MOTP: {motp:.2f}m")
        print(f"{'=' * 60}")
