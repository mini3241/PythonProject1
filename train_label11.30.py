...
def train_epoch(model, dataloader, optimizer, criterion, device, scaler, epoch,
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        try:
            ...
            # 🔧 MOTA修复v3: 只评估当前批次，不累积跨场景轨迹
            # 问题原因: 累积的轨迹来自不同场景，跨场景匹配导致FP暴增
            # 解决方案: 每次只评估当前批次的轨迹
            ...
            if eval_mota and eval_interval and batch_idx % eval_interval == 0:
                with torch.no_grad():
                    ...
                    # 🔧 关键修复: 只保留当前批次的轨迹用于MOTA计算
                    # 同时累积到all_*用于epoch结束时的总体评估
            ...
            # 🔧 匹配阈值设置
            if epoch <= 5:
                match_threshold = 10.0  # 训练初期用10米
            elif epoch <= 15:
                match_threshold = 5.0  # 中期用5米
            else:
                match_threshold = 2.5  # 后期用2.5米（baseline标准）

            if new_tracks_collected and len(pred_tracks) > 0 and len(gt_tracks) > 0:
                # 🔧 关键修复: 只用当前批次的轨迹计算MOTA，不用累积的
                is_first_mota = batch_idx <= eval_interval
                batch_mota, batch_motp = compute_mota_motp(
                    pred_tracks, gt_tracks,  # 🔧 改为只用当前批次
                    threshold=match_threshold, verbose=is_first_mota
                )
    ...
    if eval_mota and len(all_pred_tracks) > 0 and len(all_gt_tracks) > 0:
        # 动态匹配阈值策略 - 参考baseline标准(2.5m)
        if epoch <= 3:
            match_threshold = 10.0  # 训练初期稍宽松
        elif epoch <= 10:
            match_threshold = 5.0  # 中期训练
        else:
            match_threshold = 2.5  # 最终收敛到baseline标准
