...
class Trainer:
    ...
    def _get_gt_from_batch(self, batch):
        """Correctly extract GT positions and IDs, handling both batch_size=1 and >1."""
        gt_pos_raw = batch['gt_positions']
        gt_ids_raw = batch['gt_ids']

        if isinstance(gt_pos_raw, list):
            # batch_size > 1: collate wraps in list, take first sample
            gt_positions = gt_pos_raw[0].cpu().numpy()
            gt_ids = gt_ids_raw[0].cpu().numpy()
        else:
            # batch_size = 1: collate returns raw tensor (M, 2)
    ...
    def validate(self, epoch: int):
        ...
        diag_skipped_empty_gt = 0
        diag_no_detection_map = 0
        diag_no_detections = 0
        diag_has_detections = 0

        with torch.no_grad():
            pbar = tqdm(self.valid_loader, desc=f"Validation {epoch}")
            for batch in pbar:
                diag_total_batches += 1

                # Extract GT correctly for both batch sizes
                gt_positions, gt_ids = self._get_gt_from_batch(batch)

                # Skip batches with empty ground truth
                if len(gt_positions) == 0 or (gt_positions.ndim == 2 and gt_positions.shape[0] == 0):
                    diag_skipped_empty_gt += 1
                    continue

                # Ensure gt_positions is 2D (M, 2)
                ...
                # Diagnostic: print output keys for first batch
                if diag_total_batches == 1:
                    ...
                    if 'detection_map' in outputs:
                        ...
                        self.logger.info(f"[DIAG] detection_map shape: {det_map.shape}, "
                                         f"min: {det_map.min().item():.4f}, max: {det_map.max().item():.4f}, "
                                         f"mean: {det_map.mean().item():.4f}")
                    else:
                        self.logger.warning(f"[DIAG] detection_map NOT in outputs!")

                # Extract detections from output
                detections = self._extract_detections(outputs)

                if 'detection_map' not in outputs:
                    diag_no_detection_map += 1

                if len(detections) == 0:
                    diag_no_detections += 1
                else:
                    diag_has_detections += 1

                # Always compute MOTA stats, even when detections are empty
                # (empty detections = all GT are false negatives)
                if len(detections) > 0:
        ...
        self.logger.info(f"[DIAG] Validation Epoch {epoch} diagnostic summary:")
        self.logger.info(f"  Total batches: {diag_total_batches}")
        self.logger.info(f"  Skipped (empty GT): {diag_skipped_empty_gt}")
        self.logger.info(f"  No detection_map in output: {diag_no_detection_map}")
        self.logger.info(f"  Batches with 0 detections: {diag_no_detections}")
        self.logger.info(f"  Batches with >0 detections: {diag_has_detections}")
    ...
    def _extract_detections(self, outputs):
        ...
        if 'detection_map' in outputs:
            ...
            # Simple peak detection (lowered from 0.5 for diagnosis)
            threshold = 0.1
