import os
import sys
import numpy as np  # 🔧 修复: 必须在文件开头导入，因为卡尔曼滤波器需要
from datetime import datetime

# ╔═══════════════════════════════════════════════════════════════════════════╗
# ║                    📝 训练日志自动备份                                      ║
# ╠═══════════════════════════════════════════════════════════════════════════╣
# ║  ENABLE_LOG_BACKUP: 是否启用日志备份                                        ║
# ║  LOG_DIR: 日志保存目录                                                      ║
# ╚═══════════════════════════════════════════════════════════════════════════╝

ENABLE_LOG_BACKUP = True  # 🎯 启用日志备份
LOG_DIR = './training_logs'  # 🎯 日志保存目录
...
# ║                    🔴 MOTA调试参数 - 在这里统一修改                         ║
# ╠═══════════════════════════════════════════════════════════════════════════╣
# ║  DEBUG_INTERVAL: 调试打印间隔(批次)，设为0禁用调试输出                       ║
# ║  ENABLE_DETAILED_DEBUG: 是否启用详细调试（检测vs GT对比）                   ║
# ╚═══════════════════════════════════════════════════════════════════════════╝

DEBUG_INTERVAL = 500  # 🎯 每500个batch打印一次（设为0禁用）
ENABLE_DETAILED_DEBUG = True  # 🎯 启用详细调试输出

# ============================================================
# 🔧 卡尔曼滤波跟踪器 - 从baseline集成
# ============================================================
...
class SimpleKalmanFilter:
    """简化版卡尔曼滤波器 - 状态: [x, y, vx, vy]"""

    def __init__(self):
        self.dt = 1.0
        self.F = np.array([[1, 0, self.dt, 0], [0, 1, 0, self.dt], [0, 0, 1, 0], [0, 0, 0, 1]], dtype=np.float32)
        self.H = np.array([[1, 0, 0, 0], [0, 1, 0, 0]], dtype=np.float32)
        self.Q = np.diag([0.1, 0.1, 0.5, 0.5]).astype(np.float32)
        self.R = np.diag([0.5, 0.5]).astype(np.float32)

    def initiate(self, x, y):
        mean = np.array([x, y, 0, 0], dtype=np.float32)
        covariance = np.diag([1, 1, 10, 10]).astype(np.float32)
    ...
    def update(self, mean, covariance, x, y):
        z = np.array([x, y], dtype=np.float32)
        S = self.H @ covariance @ self.H.T + self.R
        K = covariance @ self.H.T @ np.linalg.inv(S)
        y_residual = z - self.H @ mean
        mean = mean + K @ y_residual
...
class KalmanTrack:
    ...
    def __init__(self, x, y, n_init=2, max_age=5):
        self.kf = SimpleKalmanFilter()
        self.mean, self.covariance = self.kf.initiate(x, y)
        ...
        # 🔧 标准做法: n_init=2，需要连续2帧匹配才确认
        # 配合SequenceSampler使用，同一序列内帧连续出现
        if self.hits >= self.n_init:
            self.state = KalmanTrackState.Confirmed
        else:
            self.state = KalmanTrackState.Tentative
    ...
    def update(self, x, y):
        self.mean, self.covariance = self.kf.update(self.mean, self.covariance, x, y)
        self.hits += 1
        self.time_since_update = 0
...
class KalmanMOTATracker:
    ...
    def update(self, detections, gt_positions=None, gt_ids=None):
        ...
        for track_idx, det_idx in matches:
            ...
            self.tracks[track_idx].update(x, y)
        ...
        for det_idx in unmatched_detections:
            x, y = detections[det_idx]
            self.tracks.append(KalmanTrack(x, y, n_init=self.n_init, max_age=self.max_age))

        self.tracks = [t for t in self.tracks if not t.is_deleted()]

        if gt_positions is not None:
            self._update_mota_stats(gt_positions, gt_ids)
    ...
    def _update_mota_stats(self, gt_positions, gt_ids=None):
        if len(gt_positions) == 0:
            return

        confirmed_tracks = [(t.track_id, t.position) for t in self.tracks if t.is_confirmed()]

        if len(confirmed_tracks) == 0:
            self.total_gt += len(gt_positions)
            self.total_fn += len(gt_positions)
            return

        cost_matrix = np.zeros((len(gt_positions), len(confirmed_tracks)), dtype=np.float32)
        for g, (gt_x, gt_y) in enumerate(gt_positions):
            for t, (track_id, track_pos) in enumerate(confirmed_tracks):
                dist = np.sqrt((track_pos[0] - gt_x) ** 2 + (track_pos[1] - gt_y) ** 2)
                cost_matrix[g, t] = dist

        cost_matrix_masked = cost_matrix.copy()
        cost_matrix_masked[cost_matrix_masked > self.max_distance] = 1e5
        row_indices, col_indices = hungarian_algorithm(cost_matrix_masked)
        ...
        for row, col in zip(row_indices, col_indices):
            if cost_matrix[row, col] <= self.max_distance:
                ...
                if gt_ids is not None:
                    gt_id = gt_ids[row]
                    track_id = confirmed_tracks[col][0]
                    if gt_id in self.gt_to_track_id:
                        if self.gt_to_track_id[gt_id] != track_id:
                            self.total_idsw += 1
                    self.gt_to_track_id[gt_id] = track_id

        self.total_gt += len(gt_positions)
        self.total_matches += len(matched_gt)
        self.total_fn += len(gt_positions) - len(matched_gt)
        self.total_fp += len(confirmed_tracks) - len(matched_tracks)
    ...
    def get_motp(self):
        """返回MOTP百分比 (0-1)，与Baseline一致"""
        if len(self.match_distances) == 0:
            return 0.0
        avg_distance = np.mean(self.match_distances)
        # Baseline公式: MOTP = 1 - (平均距离误差 / 最大匹配距离)
...
class SequenceMOTATracker:
    ...
    def update(self, group_name, frame_idx, detections, gt_positions=None, gt_ids=None):
        ...
        return tracker.update(detections, gt_positions, gt_ids)
...
# ============================================================
# 🛡️ 双保险1: 检测质量监控（不依赖跟踪器）
# ============================================================
def compute_detection_metrics(predictions, gt_positions, max_distance=5.0):
    """
    计算纯检测指标（不依赖跟踪器）

    这是MOTA的"地基"：如果检测本身不准，MOTA不可能高

    Args:
        predictions: [(x, y, conf), ...] 预测检测
        gt_positions: [(x, y), ...] GT位置
        max_distance: 匹配阈值（米）

    Returns:
        dict: {
            'recall': 检测召回率（GT被检测到的比例）
            'precision': 检测精确率（预测中正确的比例）
            'avg_distance': 匹配成功的平均距离
            'matched': 匹配成功数
            'fn': 漏检数
            'fp': 误检数
        }
    """
    if len(gt_positions) == 0:
        return {'recall': 1.0, 'precision': 1.0 if len(predictions) == 0 else 0.0,
                'avg_distance': 0.0, 'matched': 0, 'fn': 0, 'fp': len(predictions)}

    if len(predictions) == 0:
        return {'recall': 0.0, 'precision': 0.0,
                'avg_distance': 0.0, 'matched': 0, 'fn': len(gt_positions), 'fp': 0}

    # 计算距离矩阵
    cost_matrix = np.zeros((len(gt_positions), len(predictions)), dtype=np.float32)
    for g, (gt_x, gt_y) in enumerate(gt_positions):
        for p, pred in enumerate(predictions):
            pred_x, pred_y = pred[0], pred[1]
            dist = np.sqrt((pred_x - gt_x) ** 2 + (pred_y - gt_y) ** 2)
            cost_matrix[g, p] = dist

    # 匈牙利匹配
    cost_matrix_masked = cost_matrix.copy()
    cost_matrix_masked[cost_matrix_masked > max_distance] = 1e5
    row_indices, col_indices = hungarian_algorithm(cost_matrix_masked)
    ...
    for row, col in zip(row_indices, col_indices):
        if cost_matrix[row, col] <= max_distance:
            ...
            distances.append(cost_matrix[row, col])

    matched = len(matched_gt)
    fn = len(gt_positions) - matched  # 漏检
    fp = len(predictions) - len(matched_pred)  # 误检

    recall = matched / len(gt_positions) if len(gt_positions) > 0 else 0.0
    precision = len(matched_pred) / len(predictions) if len(predictions) > 0 else 0.0
...
# 🛡️ 双保险2: 坐标系早期验证
# ============================================================
def verify_coordinate_system(pred_positions, gt_positions, batch_idx=0):
    """
    验证预测和GT是否在同一坐标系

    如果坐标系错了，预测和GT的距离会非常大（>50米）

    Returns:
        bool: True=坐标系正确, False=坐标系可能有问题
        str: 诊断信息
    """
    if len(pred_positions) == 0 or len(gt_positions) == 0:
        return True, "无法验证（数据为空）"
    ...
    # 计算中心点距离
    center_distance = np.sqrt((pred_center[0] - gt_center[0]) ** 2 +
    ...
    # 诊断
    diagnosis = []
    diagnosis.append(f"预测中心: ({pred_center[0]:.1f}, {pred_center[1]:.1f})")
    diagnosis.append(f"GT中心: ({gt_center[0]:.1f}, {gt_center[1]:.1f})")
    diagnosis.append(f"中心距离: {center_distance:.1f}m")
    diagnosis.append(f"最近距离: {min_distance:.1f}m")

    # 判断坐标系是否正确
    if center_distance > 50 or min_distance > 30:
        diagnosis.append("❌ 坐标系可能有问题！预测和GT相差太远")
        return False, "\n   ".join(diagnosis)
    elif center_distance > 20 or min_distance > 15:
        diagnosis.append("⚠️ 坐标系可能有偏移，建议检查")
        return True, "\n   ".join(diagnosis)
    else:
        diagnosis.append("✅ 坐标系看起来正确")
        return True, "\n   ".join(diagnosis)
...
class TrainingMonitor:
    ...
    def __init__(self, warning_threshold_recall=0.3, warning_epoch=5):
        self.warning_threshold_recall = warning_threshold_recall
        self.warning_epoch = warning_epoch
        self.epoch_recalls = []
        self.epoch_precisions = []
        self.epoch_distances = []
        self.coordinate_verified = False
        self.has_warned = False

    def update(self, recall, precision, avg_distance):
        self.epoch_recalls.append(recall)
        self.epoch_precisions.append(precision)
        self.epoch_distances.append(avg_distance)

    def check_and_warn(self, epoch):
        """检查训练状态，必要时发出警告"""
        if self.has_warned or epoch < self.warning_epoch:
            return

        # 计算最近几个epoch的平均Recall
        recent_recalls = self.epoch_recalls[-self.warning_epoch:]
        avg_recall = np.mean(recent_recalls) if recent_recalls else 0.0

        if avg_recall < self.warning_threshold_recall:
            print(f"\n{'🚨' * 30}")
            print(f"🚨 训练早期预警！")
            print(f"{'🚨' * 30}")
            print(f"\n前{self.warning_epoch}个epoch平均检测Recall只有 {avg_recall * 100:.1f}%")
            print(f"这意味着模型检测不到大部分目标！")
            print(f"\n可能的原因：")
            print(f"  1. 坐标系不对齐（预测位置和GT位置差很远）")
            print(f"  2. 检测阈值太高（score_threshold）")
            print(f"  3. Loss没有正常下降（检查loss曲线）")
            print(f"  4. 数据预处理有问题（BEV输入不正确）")
            print(f"\n建议：")
            print(f"  - 检查DEBUG输出中的预测位置 vs GT位置")
            print(f"  - 尝试降低score_threshold到0.1")
            print(f"  - 确认BEV坐标范围设置正确")
            print(f"{'🚨' * 30}\n")
            self.has_warned = True

    def print_summary(self, epoch):
        """打印epoch总结"""
        if len(self.epoch_recalls) == 0:
            return

        recall = self.epoch_recalls[-1]
        precision = self.epoch_precisions[-1]
        avg_dist = self.epoch_distances[-1]

        print(f"\n📊 检测质量指标（独立于跟踪器）:")
        print(f"   Recall(召回率): {recall * 100:.1f}% - GT被检测到的比例")
        print(f"   Precision(精确率): {precision * 100:.1f}% - 预测中正确的比例")
        print(f"   平均匹配距离: {avg_dist:.2f}m")

        # 给出建议
        if recall < 0.3:
            print(f"   ⚠️ Recall太低！大部分GT没被检测到")
        elif recall < 0.5:
            print(f"   🟡 Recall偏低，还有提升空间")
        else:
            print(f"   ✅ Recall良好")

        if precision < 0.3:
            print(f"   ⚠️ Precision太低！太多误检")
        elif precision < 0.5:
            print(f"   🟡 Precision偏低，误检较多")
        else:
            print(f"   ✅ Precision良好")


# 全局监控器实例
training_monitor = TrainingMonitor(warning_threshold_recall=0.3, warning_epoch=5)
...
class SequenceSampler(Sampler):
    ...
    特点:
    - 序列之间shuffle（随机选择下一个处理哪个序列）
    - 序列内部按帧顺序（同一目标连续出现，支持n_init=2）

    这样既保持一定随机性，又能让跟踪器正常工作
    """

    def __init__(self, dataset, shuffle_groups=True):
        """
        Args:
            dataset: MultiModalLabeledDataset实例
            shuffle_groups: 是否打乱序列顺序（推荐True）
        """
        self.dataset = dataset
        self.shuffle_groups = shuffle_groups

        # 从dataset的samples中提取group信息
        samples_df = dataset.samples.copy().reset_index(drop=True)

        # 从relpath中提取group名称
        # 例如: "20221217_group0000_mode1_279frames/frame0000" -> "20221217_group0000_mode1_279frames"
        samples_df['group'] = samples_df['relpath'].apply(
            lambda x: x.split('/')[0] if '/' in str(x) else 'default'
        )

        # 从relpath中提取帧号
        # 例如: "xxx/frame0000" -> 0
        def extract_frame_num(relpath):
            try:
                frame_part = relpath.split('/')[-1] if '/' in str(relpath) else relpath
                frame_num = int(frame_part.replace('frame', '').replace('_', ''))
                return frame_num
            except:
                return 0

        samples_df['frame_num'] = samples_df['relpath'].apply(extract_frame_num)

        # 保存原始索引（dataset中的位置）
        samples_df['dataset_idx'] = range(len(samples_df))

        # 按group分组，每个group内按帧号排序
        self.groups = {}
        for group_name in samples_df['group'].unique():
            group_df = samples_df[samples_df['group'] == group_name].sort_values('frame_num')
            self.groups[group_name] = group_df['dataset_idx'].tolist()

        self.group_names = list(self.groups.keys())

        print(f"📊 SequenceSampler初始化完成:")
        print(f"   共{len(self.group_names)}个视频序列")
        print(f"   共{len(samples_df)}个样本")
        print(f"   序列间shuffle: {shuffle_groups}")
        print(f"   序列内按帧顺序: True")

    def __iter__(self):
        # 打乱序列顺序（每个epoch不同）
        group_order = self.group_names.copy()
...
class MultiModalLabeledDataset(Dataset):
    ...
    def _load_camera_data(self, frame_path):
        """加载相机数据和标签 - 🔧 修复：将GT坐标从LiDAR坐标系转换到OCU雷达坐标系"""
        ...
        ocu_path = os.path.join(frame_path, 'OCULiiRadar')  # 🔧 新增：OCU目录
        targets = []
        label_data = {}
        ocu_data = {}  # 🔧 新增：存储OCU JSON数据
        ...
        if os.path.exists(lidar_path):
            ...
            if json_files:
                ...
                # 🔧 关键修复：从OCULiiRadar目录读取OCU extrinsic
                if os.path.exists(ocu_path):
                    ocu_json_files = [f for f in os.listdir(ocu_path) if f.endswith('.json')]
                    if ocu_json_files:
                        ocu_json_path = os.path.join(ocu_path, ocu_json_files[0])
                        with open(ocu_json_path, 'r') as f:
                            ocu_data = json.load(f)

                annotations = label_data.get('annotation', [])

                # 🔧 计算LiDAR→OCU转换矩阵（修复：从两个不同目录的JSON读取）
                transform_matrix = None
                try:
                    # 🔧 关键修复：Lidar extrinsic在VelodyneLidar JSON，OCU extrinsic在OCULiiRadar JSON
                    lidar_has_extrinsic = 'VelodyneLidar_to_LeopardCamera0_extrinsic' in label_data
                    ocu_has_extrinsic = 'OCULiiRadar_to_LeopardCamera0_extrinsic' in ocu_data

                    if lidar_has_extrinsic and ocu_has_extrinsic:
                        Lidar_to_Camera = np.array(label_data['VelodyneLidar_to_LeopardCamera0_extrinsic'])
                        OCU_to_Camera = np.array(ocu_data['OCULiiRadar_to_LeopardCamera0_extrinsic'])
                        transform_matrix = np.matmul(np.linalg.inv(OCU_to_Camera), Lidar_to_Camera)
                except Exception as e:
                    transform_matrix = None

                # 🔴 调试：只在第一次打印
                if not hasattr(self, '_gt_transform_debug_done'):
                    self._gt_transform_debug_done = True
                    print(f"\n🔴🔴🔴 GT坐标转换调试 🔴🔴🔴")
                    print(
                        f"   VelodyneLidar JSON有Lidar_extrinsic: {'VelodyneLidar_to_LeopardCamera0_extrinsic' in label_data}")
                    print(
                        f"   OCULiiRadar JSON有OCU_extrinsic: {'OCULiiRadar_to_LeopardCamera0_extrinsic' in ocu_data}")
                    print(f"   transform_matrix计算成功: {transform_matrix is not None}")
                    if transform_matrix is None:
                        print(f"   ⚠️ GT坐标将使用原始LiDAR坐标（未转换）!")
                        print(f"   label_data的keys: {list(label_data.keys())[:10]}...")
                        print(f"   ocu_data的keys: {list(ocu_data.keys())[:10]}...")
                    else:
                        print(f"   ✅ GT坐标转换已启用！LiDAR→OCU转换矩阵已计算")
                    print(f"🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴🔴\n")
                ...
                for ann in annotations:
                    ...
                    if ann.get('class', '') in ['car', 'bus']:
                        # 原始LiDAR坐标
                        lidar_x = float(ann.get('x', 0))
                        lidar_y = float(ann.get('y', 0))
                        lidar_z = float(ann.get('z', 0))

                        # 🔧 关键修复：将GT从LiDAR坐标系转换到OCU坐标系
                        if transform_matrix is not None:
                            # 构建点云格式 (3, 1)
                            point = np.array([[lidar_x], [lidar_y], [lidar_z]])
                            point1 = np.vstack((point, np.ones((1, 1))))  # (4, 1)
                            point_ocu = np.matmul(transform_matrix, point1)[:3, 0]  # (3,)

                            # Baseline的坐标交换：OCU的Z变成Y（前向距离）
                            ocu_x = point_ocu[0]  # 横向
                            ocu_y = point_ocu[2]  # 前向距离（OCU的Z → BEV的Y）
                            ocu_z = point_ocu[1]  # 高度（OCU的Y → BEV的Z）
                        else:
                            # 如果没有转换矩阵，使用原始坐标
                            ocu_x = lidar_x
                            ocu_y = lidar_y
...
def compute_mota_motp(pred_tracks, gt_tracks, threshold=5.0, verbose=False, max_range=None):
    ...
    try:
        ...
        if total_gt > 0:
            ...
            # ═══════════════════════════════════════════════════════════════
            print(f"\n{'═' * 70}")
            print(f"🔴 断点3: MOTA计算详情 [匹配阈值={threshold}m]")
            print(f"{'═' * 70}")
            print(f"📊 核心统计:")
            print(f"   总GT数:     {total_gt}")
            print(f"   总预测数:   {total_matches + total_fp}")
            print(f"   总匹配数:   {total_matches} (TP)")
            print(f"   总FP数:     {total_fp} (假阳性-多检测)")
            print(f"   总FN数:     {total_fn} (假阴性-漏检测)")
            print(f"   总IDSW数:   {total_idsw} (ID切换)")

            print(f"\n📈 MOTA分解:")
            print(f"   总错误 = FP + FN + IDSW = {total_fp} + {total_fn} + {total_idsw} = {total_errors}")
            print(f"   错误率 = {total_errors}/{total_gt} = {total_errors / total_gt:.2%}")
            mota_value = max(0.0, 1.0 - total_errors / total_gt)
            print(f"   MOTA   = 1 - {total_errors / total_gt:.4f} = {mota_value:.4f} ({mota_value * 100:.2f}%)")

            # 诊断建议
            print(f"\n💡 诊断建议:")
            if total_fp > total_gt * 0.5:
                print(f"   ⚠️ FP过高({total_fp}>{total_gt * 0.5:.0f})! 建议提高score_threshold")
            else:
                print(f"   ✅ FP正常")
            if total_fn > total_gt * 0.3:
                print(f"   ⚠️ FN过高({total_fn}>{total_gt * 0.3:.0f})! 建议降低score_threshold或检查坐标系")
            else:
                print(f"   ✅ FN正常")
            if total_matches < total_gt * 0.5:
                print(f"   ⚠️ 匹配率过低({total_matches}/{total_gt}={total_matches / total_gt:.1%})! 检查坐标转换")
            else:
                print(f"   ✅ 匹配率正常 ({total_matches / total_gt:.1%})")
            if total_errors <= total_gt * 0.65:
                print(f"   🎯 MOTA预期 >= 35%")
            print(f"{'═' * 70}")
        ...
        for batch_idx in range(batch_size):
            try:
                ...
                if len(output.shape) >= 2:
                    ...
                    score_threshold = 0.3  # 🔧 恢复到0.3（0.5太高导致漏检）
                    ...
                    if len(coords[0]) > 0:
                        ...
                        for det_idx, (y, x) in enumerate(zip(coords[0], coords[1])):
                            ...
                            # 🔴🔴🔴 MOTA诊断断点 - 自动插入 🔴🔴🔴
                            # 使用DEBUG_INTERVAL控制打印频率
                            should_print_debug = (
                                    DEBUG_INTERVAL > 0 and batch_num % DEBUG_INTERVAL == 0 and det_idx == 0 and len(
                                targets) > 0)

                            if should_print_debug and ENABLE_DETAILED_DEBUG:
                                gt_x = targets[0].get('x', 0)
                                gt_y = targets[0].get('y', 0)
                                dist_to_gt = np.sqrt((world_x - gt_x) ** 2 + (world_y - gt_y) ** 2)

                                print(f"\n{'═' * 70}")
                                print(f"🔴 断点2: 检测 vs GT 对比 - Batch {batch_num}")
                                print(f"{'═' * 70}")

                                # 统计信息
                                print(f"📊 统计:")
                                print(f"   检测数: {len(final_detections) + 1}")  # +1因为当前还没append
                                print(f"   GT数:   {len(targets)}")
                                if len(targets) > 0:
                                    ratio = (len(final_detections) + 1) / len(targets)
                                    status = "✅正常" if 0.8 <= ratio <= 1.5 else "⚠️FP多" if ratio > 1.5 else "⚠️FN多"
                                    print(f"   比值:   {ratio:.2f} {status}")

                                print(f"\n📍 第一个检测 vs 第一个GT:")
                                print(f"   预测: x={world_x:.2f}m, y={world_y:.2f}m (conf={confidence:.3f})")
                                print(f"   GT:   x={gt_x:.2f}m, y={gt_y:.2f}m")
...
def train_epoch(model, dataloader, optimizer, criterion, device, scaler, epoch,
    ...
    # 🔧 使用卡尔曼滤波跟踪器计算MOTA（和baseline一致）
    # 🔴 修复：增大匹配阈值，让MOTA能正确反映学习进度
    if epoch <= 5:
        kalman_match_threshold = 15.0  # 训练初期用15米
    elif epoch <= 15:
        kalman_match_threshold = 10.0  # 中期用10米
    else:
        kalman_match_threshold = 5.0  # 后期用5米

    sequence_tracker = SequenceMOTATracker(
        max_age=5,  # baseline默认值
        n_init=2,  # 🔧 标准做法：需要连续2帧匹配才确认
        max_distance=kalman_match_threshold  # 根据epoch调整
    )
    print(f"   📊 MOTA评估使用卡尔曼跟踪器 (max_age=5, n_init=2, threshold={kalman_match_threshold}m)")
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        try:
            ...
            # 🔧 MOTA修复v4: 使用卡尔曼滤波跟踪器（和baseline完全一致）
            # 优势: 正确计算ID Switch，序列级别跟踪，卡尔曼预测平滑
            ...
            if eval_mota and eval_interval and batch_idx % eval_interval == 0:
                with torch.no_grad():
                    ...
                    MAX_ACCUMULATED = 500
                    if len(all_pred_tracks) > MAX_ACCUMULATED:
                        all_pred_tracks = all_pred_tracks[-MAX_ACCUMULATED:]
                    if len(all_gt_tracks) > MAX_ACCUMULATED:
                        all_gt_tracks = all_gt_tracks[-MAX_ACCUMULATED:]
                    ...
                    if len(pred_tracks) > 0 and len(gt_tracks) > 0:
                        ...
                        if len(targets) > 0 and isinstance(targets[0], dict):
                            group_name = targets[0].get('groupname', 'default')

                        # 按帧组织数据
                        frames_data = {}
                        for track in pred_tracks:
                            frame_id = str(track[0])
                            x, y = track[2], track[3]
                            if frame_id not in frames_data:
                                frames_data[frame_id] = {'pred': [], 'gt': [], 'gt_ids': []}
                            frames_data[frame_id]['pred'].append((x, y))
                        ...
                        for track in gt_tracks:
                            ...
                            if frame_id not in frames_data:
                                frames_data[frame_id] = {'pred': [], 'gt': [], 'gt_ids': []}
                            frames_data[frame_id]['gt'].append((x, y))
                            frames_data[frame_id]['gt_ids'].append(track_id)

                        # 按帧顺序更新跟踪器
                        for frame_id in sorted(frames_data.keys()):
                            data = frames_data[frame_id]
                            try:
                                frame_idx = int(frame_id.replace('frame', '').replace('_', ''))
                            except:
                                frame_idx = hash(frame_id) % 10000

                            sequence_tracker.update(
                                group_name=group_name,
                                frame_idx=frame_idx,
                                detections=data['pred'],
                                gt_positions=data['gt'],
                                gt_ids=data['gt_ids']
                            )
                        ...
                        # 收集所有预测和GT用于检测质量评估
                        all_pred_positions = []
                        all_gt_positions_flat = []
                        for frame_id, data in frames_data.items():
                            for pred in data['pred']:
                                all_pred_positions.append((pred[0], pred[1], 1.0))
                            for gt in data['gt']:
                                all_gt_positions_flat.append(gt)

                        # 计算检测指标
                        det_metrics = compute_detection_metrics(
                            all_pred_positions,
                            all_gt_positions_flat,
                            max_distance=kalman_match_threshold
                        )

                        # 🛡️ 累积检测指标
                        epoch_total_matched += det_metrics['matched']
                        epoch_total_gt += len(all_gt_positions_flat)
                        epoch_total_pred += len(all_pred_positions)
                        epoch_total_fn += det_metrics['fn']
                        epoch_total_fp += det_metrics['fp']
                        if det_metrics['avg_distance'] > 0:
                            epoch_distances.append(det_metrics['avg_distance'])

                        # 🛡️ 双保险2: 坐标系验证（只在第一个batch执行）
                        if batch_idx == 0 and not training_monitor.coordinate_verified:
                            pred_xy = [(p[0], p[1]) for p in all_pred_positions]
                            coord_ok, coord_diag = verify_coordinate_system(
                                pred_xy, all_gt_positions_flat, batch_idx
                            )
                            print(f"\n{'=' * 60}")
                            print(f"🛡️ 坐标系验证 (Batch 0):")
                            print(f"   {coord_diag}")
                            print(f"{'=' * 60}\n")
                            training_monitor.coordinate_verified = True

                            if not coord_ok:
                                print(f"{'🚨' * 30}")
                                print(f"🚨 坐标系可能有问题！请检查BEV坐标范围设置")
                                print(f"{'🚨' * 30}\n")

            # 获取卡尔曼跟踪器的MOTA
            if new_tracks_collected:
                batch_mota = sequence_tracker.get_global_mota()
                batch_motp = sequence_tracker.get_global_motp()

                # 输出
                if eval_interval and batch_idx % eval_interval == 0:
                    # 获取当前统计
                    if sequence_tracker.current_group in sequence_tracker.trackers:
                        tracker = sequence_tracker.trackers[sequence_tracker.current_group]
                        total_idsw = sequence_tracker.global_idsw + tracker.total_idsw
                    else:
                        total_idsw = sequence_tracker.global_idsw

                    print(
                        f"\n   📊 批次{batch_idx} | MOTA={batch_mota * 100:.2f}%, MOTP={batch_motp * 100:.2f}%, IDSW={total_idsw}")
        ...
        # 添加MOTA/MOTP到进度条（显示当前批次的MOTA，而非累积的）
        if new_tracks_collected and len(pred_tracks) > 0 and len(gt_tracks) > 0:
            # 🔧 修复: 显示当前批次的MOTA，不是累积的
            postfix_dict['MOTA'] = f"{batch_mota:.2f}"
            postfix_dict['MOTP'] = f"{batch_motp:.2f}"
    ...
    if eval_mota:
        print(f"\n{'=' * 60}")
        print(f"📊 Epoch {epoch} MOTA统计（卡尔曼跟踪器）")
        sequence_tracker.print_global_metrics()
        mota = sequence_tracker.get_global_mota()
        motp = sequence_tracker.get_global_motp()
        print(f"   ✅ 最终 MOTA: {mota * 100:.2f}%, MOTP: {motp * 100:.2f}%")
        print(f"{'=' * 60}")

        # 🛡️ 双保险: 打印检测质量指标
        if epoch_total_gt > 0:
            epoch_recall = epoch_total_matched / epoch_total_gt
            epoch_precision = epoch_total_matched / epoch_total_pred if epoch_total_pred > 0 else 0.0
            epoch_avg_dist = np.mean(epoch_distances) if epoch_distances else 0.0

            print(f"\n{'=' * 60}")
            print(f"🛡️ 双保险: 检测质量指标（独立于跟踪器）")
            print(f"{'=' * 60}")
            print(f"   📊 Recall(召回率): {epoch_recall * 100:.1f}% ({epoch_total_matched}/{epoch_total_gt})")
            print(f"   📊 Precision(精确率): {epoch_precision * 100:.1f}% ({epoch_total_matched}/{epoch_total_pred})")
            print(f"   📊 平均匹配距离: {epoch_avg_dist:.2f}m")
            print(f"   📊 漏检(FN): {epoch_total_fn}, 误检(FP): {epoch_total_fp}")

            # 给出建议
            if epoch_recall < 0.3:
                print(f"\n   ⚠️ Recall太低(<30%)！大部分GT没被检测到")
                print(f"      → MOTA不可能高，需要检查：")
                print(f"         1. 坐标系是否对齐？")
                print(f"         2. Loss是否在下降？")
                print(f"         3. 检测阈值是否太高？")
            elif epoch_recall >= 0.5:
                print(f"\n   ✅ Recall良好(≥50%)，检测基础OK")

            # 更新训练监控器
            training_monitor.update(epoch_recall, epoch_precision, epoch_avg_dist)
            training_monitor.check_and_warn(epoch)
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=20, help='训练轮数(优化为20轮以加速)')
    ...
    print(f"{'═' * 70}")
    print(f"📊 检测参数:")
    print(f"   score_threshold = 0.3  (置信度阈值)")
    print(f"   max_num_objects = 15   (最大检测数)")
    print(f"📊 匹配阈值 (根据epoch自动调整):")
    print(f"   epoch 1-5:   15.0m")
    print(f"   epoch 6-15:  10.0m")
    print(f"   epoch 16+:   5.0m")
    print(f"📊 调试设置:")
    print(f"   DEBUG_INTERVAL = {DEBUG_INTERVAL}  (每N个batch打印调试信息)")
    print(f"   ENABLE_DETAILED_DEBUG = {ENABLE_DETAILED_DEBUG}")
    print(f"{'═' * 70}")
    print(f"💡 调参提示:")
    print(f"   - 如果FP多(检测数/GT>1.5): 提高score_threshold")
    print(f"   - 如果FN多(检测数/GT<0.8): 降低score_threshold")
    print(f"   - 训练初期MOTA低是正常的，需要20+ epoch才能看到效果")
    print(f"{'═' * 70}\n")
    print()
    ...
    # 🔧 创建按序列采样器（标准MOT训练做法）
    sequence_sampler = SequenceSampler(dataset, shuffle_groups=True)
    print(f"\n✅ 使用SequenceSampler（序列间shuffle，序列内按帧顺序）")
    print(f"   这样n_init=2可以正常工作，MOTA计算有意义\n")
    ...
    def create_dataloader(batch_size):
        return DataLoader(
            ...
            sampler=sequence_sampler,  # 🔧 按序列采样
    ...
    with torch.no_grad():
        # 创建虚拟输入 - 🔧 修复：形状必须和实际训练时一致！
        dummy_batch_size = 2
        dummy_H, dummy_W = 640, 640
        dummy_inputs = {
            # detector期待的格式: (B, H, W, C)
            'dynamic_HM': torch.randn(dummy_batch_size, dummy_H, dummy_W, 3).to(device),  # (B, H, W, 3)
            'static_HM': torch.randn(dummy_batch_size, dummy_H, dummy_W, 3).to(device),  # (B, H, W, 3)
            # SGDNet输入
            'input_img': torch.randn(dummy_batch_size, 3, 640, 640).to(device),  # (B, 3, H, W) RGB图像
            'input_radar': torch.randn(dummy_batch_size, 640, 640).to(device),  # (B, H, W) 雷达深度图
            'input_velo': torch.randn(dummy_batch_size, 640, 640).to(device),  # (B, H, W) LiDAR深度图
            'input_knn': torch.zeros(dummy_batch_size, 6, 640, 640).to(device),  # (B, 6, H, W) KNN
            'segmap': torch.zeros(dummy_batch_size, 6, 640, 640).to(device),  # (B, 6, H, W) 分割图
            'oculii_img': torch.randn(dummy_batch_size, 5, 640, 640).to(device),  # (B, 5, H, W) OCULii BEV
        }
        # 让segmap的class 0有值（和实际训练一致）
        dummy_inputs['segmap'][:, 0, :, :] = 1.0
